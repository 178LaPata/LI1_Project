...
printMaze l = foldr (\x a-> printCorridor x ++ a) "\n" l
-}
...
parfimaux [] ta = []
parfimaux l@(h:t) ta 
    | odd ta  = if length l == metade+1 then (8:tiral++[8]):par
                else (71:tiral++[71]):par
    | even ta = if length l == metade || length l == metade+1 then (8:tiral++[8]):par
                else (71:tiral++[71]):par
...
casaaux l@(h:t) al co
    ...
    | otherwise = if length l <= metade+3 then [colocaCorridor h co]++casaaux t al (co+1) 
